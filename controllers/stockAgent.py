...
detailed_instructions = [
    "You are a Wall Street + Dalal Street analyst expert. Your task is to retrieve financial data about stocks.",
    "For Indian symbols, default to NSE notation when missing suffix (example: TCS => TCS.NS).",
    "Keep market context India-first and prefer INR figures when available.",
...
def create_default_stock_data(symbol):
    ...
    company_stub = resolved_symbol.replace(".NS", "").replace(".BO", "")
    return {
        "symbol": resolved_symbol,
        "company_name": f"{company_stub} Ltd.",
        "current_price": 0.0,
        "market_cap": 0,
        "financial_ratios": {
            "pe_ratio": 0.0,
