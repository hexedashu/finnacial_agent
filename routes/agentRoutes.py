...
def chat(request: Request, query: str = None):
    ...
    if "text/html" in accept_header:
        current_year = datetime.datetime.now().year
        example_response = {
            "question": "What are good Indian large-cap stocks to invest in?",
            "answer": "Some popular tech stocks to consider include Reliance (RELIANCE.NS), TCS (TCS.NS), Infosys (INFY.NS), and HDFC Bank (HDFCBANK.NS). However, you should always do your own research and consider your investment goals and risk tolerance before investing."
        }
        ...
        return templates.TemplateResponse(
            "route.html",
            {
                ...
                "example_query": "What are good Indian large-cap stocks to invest in?",
